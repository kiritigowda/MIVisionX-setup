...
parser.add_argument('--directory',  type=str, default='',
                    help='MIVisionX-profile.py directory - optional (default:~/)')
parser.add_argument('--mode',       type=int, default=1,
                    help='MIVisionX-profile.py mode used - optional (default: 1)')
...
if __name__ == "__main__":
    ...
    if profileMode == 1 or profileMode == 2 or profileMode == 3:
        path_to_so = MIVisionX_dev_dir + \
            '/develop/caffe-folder/googlenet/nnir_build_1/libannmodule.so'
    else:
    ...
    with open(report_filename, 'w') as f:
        ...
        if profileMode == 1:
            ...
            profileReportFile = MIVisionX_dev_dir + \
                '/develop/caffe2nnir2openvx_noFuse_profile.md'
        if profileMode == 2:
            f.write("\n CAFFE to NNIR to OpenVX - Fuse profile \n\n")
            profileReportFile = MIVisionX_dev_dir + \
                '/develop/caffe2nnir2openvx_fuse_profile.md'
        if profileMode == 3:
            f.write("\n CAFFE to NNIR to OpenVX - FP16 profile \n\n")
            profileReportFile = MIVisionX_dev_dir + \
                '/develop/caffe2nnir2openvx_fp16_profile.md'
        if profileMode == 4:
            f.write("\n ONNX to NNIR to OpenVX - noFuse profile \n\n")
            profileReportFile = MIVisionX_dev_dir + \
                '/develop/onnx2nnir2openvx_noFuse_profile.md'
        if profileMode == 5:
        ...
        if profileMode == 7:
            ...
            profileReportFile = MIVisionX_dev_dir + \
                '/develop/nnef2nnir2openvx_noFuse_profile.md'
        if profileMode == 8:
    ...
    if(os.path.exists(diffFolder_dir)):
        ...
        os.system('diff -y --suppress-common-lines '+report_filename+' '+diffFolder_dir +
                  '/latestReportFile.md | aha --black --title '+titleName+' > reportDiff.html')
        os.system(' cp '+report_filename+' ' +
                  diffFolder_dir+'/latestReportFile.md')
    else:
